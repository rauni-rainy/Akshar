...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      if (isGoogleFlow) {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}`}/api/auth/google/complete`, {
          method: "POST",
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}`}/api/auth/google`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
