...
export default function PublicProfile() {
  ...
  const fetchPublicProfile = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/users/${id}`, { headers });
      if (res.ok) {
  ...
  const handleFollowToggle = async () => {
    ...
    try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/users/${id}/follow`, {
            method,
            headers: {
