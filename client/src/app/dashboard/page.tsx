...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (draftIdParam) {
          ...
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/posts/${draftIdParam}`)
            .then(res => res.json())
            .then(data => {
                if(data && !data.error) {
                    setTitle(data.title || "");
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}`}/api/users/me`, {
  ...
  const fetchPosts = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/posts/feed?limit=1000`);
      if (res.ok) {
  ...
  const saveDraft = async (manual = false) => {
    ...
    try {
      let url = `${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}`}/api/posts`;
      let method = "POST";
      
      if (postId) {
         url = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/posts/${postId}`;
         method = "PUT";
      }
